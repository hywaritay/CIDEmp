...
import { createItem, getItems, updateItem, deleteItem } from '../api/locationapi'; 
...
export default function DepartmentComponent() {
  ...
  const handleEdit = (item) => {
    setSelectedItem(item);
    setNameUpdate(item.district);
    setModalVisible(true);
  };

  const handleSaveEdit = async () => {
    if (selectedItem) {
      console.log(`Edited location: ${name}`);
      await updateItem({ 
        id: selectedItem.id, 
        district: name, 
        active: true,
  ...
  return (
    <View style={{ marginTop: 30 }}>
      <Text style={{ fontFamily: 'outfit-bold', color: '#9586A8', marginBottom: 5, marginLeft: 36 }}>
        {'Enter District Name :'}
      </Text>
      <View>
        <TextInput
